...
from micode.memory.entity import extract_entities_and_relations
from micode.models import EventType, Run, StepType
from micode.memory.episodic import EpisodicMemoryStore, episodic_memory_from_trace
from micode.memory.graph import MemoryGraphStore, build_memory_graph
...
def run_agent_task(
    ...
    prompt_cache_dir: str = ".micode/prompt-cache",
...
def resolve_cli_skill_candidate_dir(memory_dir: str, candidate_dir: str) -> str:
    ...
    if (
        candidate_dir == ".micode/skill-candidates"
...
def main() -> None:
    ...
    fixed_parser.add_argument("--trace-dir", default=".micode/traces")
    ...
    agent_parser.add_argument("--trace-dir", default=".micode/traces")
    agent_parser.add_argument("--session-id", default="")
    agent_parser.add_argument("--session-dir", default=".micode/sessions")
    agent_parser.add_argument("--session-title", default="")
    agent_parser.add_argument("--memory-dir", default=".micode/memory")
    ...
    agent_parser.add_argument("--artifact-dir", default=".micode/artifacts")
    agent_parser.add_argument("--artifact-threshold-chars", type=int, default=8000)
    agent_parser.add_argument("--prompt-cache-dir", default=".micode/prompt-cache")
    agent_parser.add_argument(
        "--skill-candidate-dir",
        default=".micode/skill-candidates",
    )
    ...
    traces_parser.add_argument("--trace-dir", default=".micode/traces")
    ...
    cleanup_parser.add_argument("--trace-dir", default=".micode/traces")
    cleanup_parser.add_argument("--keep", type=int, default=20)

    memory_review_parser = subparsers.add_parser("memory-review")
    memory_review_parser.add_argument("--session-dir", default=".micode/sessions")
    memory_review_parser.add_argument("--memory-dir", default=".micode/memory")
    ...
    skill_candidate_parser.add_argument("--candidate-dir", default=".micode/skill-candidates")
    skill_candidate_parser.add_argument("--memory-dir", default=".micode/memory")
    skill_candidate_parser.add_argument("--skills-root", default=".micode/skills")
    ...
    migrate_parser = subparsers.add_parser(
        "migrate-state",
        help="copy legacy .minicode state into .micode with SHA-256 verification",
    )
    ...
    if args.command == "fixed":
        ...
        report = migrate_state(args.source, args.target)
        print(json.dumps(report, ensure_ascii=False, indent=2))
        if not report["ok"]:
            raise SystemExit(1)
